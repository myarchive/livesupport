	...
	// Admin pages
		...
		if (this.params.a === 'users') {
			if (this.params.b === undefined) {
				...
				Session.set('currPage', 'adminUsersView');
				var user = Meteor.users.findOne({username: this.params.b});
				if (user === undefined)
					this.render('adminUsersNoUser', {data: {username: this.params.b}});
				else {
					if (this.params.c === undefined)
						this.render('adminUsersView', {data: user});
					else if (this.params.c === 'edit')
						this.render('adminUsersEdit', {data: user});
					else
